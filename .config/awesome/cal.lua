	...
	if not tooltip then
		tooltip = awful.tooltip({})
                function tooltip:update()
                        ...
                        state = {month, year}
                        tooltip:set_text(string.format('<span font_desc="Droid Sans Mono">%s</span>', displayMonth(month, year, 2)))
                end
                tooltip:update()
	end
...
function switchMonth(delta)
	...
	local text = string.format('<span font_desc="Droid Sans Mono">%s</span>', displayMonth(state[1], state[2], 2))
